...
杀手锏交易系统 V6.1 - 统一入口
支持四种运行模式：
  - v61: V6.1 总控中心版（推荐，含自我检查/自我修复/自我优化）
  - v60: V6.0 智能优化版（EV过滤+订单生命周期管理）
...
def run_v61_mode(args):
    ...
    核心特性：
    - V6.0: EV过滤 + 订单生命周期管理
    - V6.1: 总控中心（全局状态/健康检查/修复引擎/任务调度/性能优化）
    - 零侵入集成：各层执行前查询全局状态
    - 风控熔断联动：软/硬熔断自动切换系统状态
    - 多symbol并行调度
    """
    print("\n" + "="*70)
    print("  杀手锏交易系统 V6.1 - 总控中心版")
    print("="*70)
    print("\n  核心能力：")
    print("  [V6.0] EV过滤：预期价值过滤，只执行正期望交易")
    print("  [V6.0] 订单生命周期管理：幂等性控制，防止重复下单")
    print("  [V6.1] 总控中心：全局状态管理 + 健康检查 + 修复引擎")
    print("  [V6.1] 任务调度：多symbol并行调度")
    print("  [V6.1] 性能优化：动态调参 + 离线搜索触发")
    print("  [V6.1] 风控熔断联动：软/硬熔断自动切换系统状态")
    print("\n  预期性能：")
    print("  - 胜率：63-65%")
    print("  - 夏普比率：1.2-1.6")
    print("  - 最大回撤：8-12%")
    ...
    from scripts.complete_loop_v61 import CompleteLoopV61
    system = CompleteLoopV61(config_path)
