...
description: 工业级杀手锏多策略融合交易系统(v1.1)；当用户需要闭环集成回测、风险平价资金分配(ERC/HERC/IVP)、Hurst指数过滤、多维评分信号、动态保本止损、期货品种支持、贝叶斯参数优化、HRP组合优化、元学习快速适应、Hawkes市场冲击建模、因果因子筛选、市场冲击模型统一接口、过拟合检测(CSCV/PBO/DSR)、DAG流水线并行编排时使用
dependency:
  python:
    - numpy>=1.24.0
    - pandas>=2.0.0
