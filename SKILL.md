...
description: 工业级杀手锏交易系统(v1.0.3)；当用户需要策略回测、风险模型验证、多策略组合优化、高频交易模拟、交易复盘分析、系统健康评估、自适应阈值优化、风控联动、智能调度时使用
dependency:
  python:
    - numpy==1.24.3
    - pandas==2.0.3
...
- 配置文件: `assets/configs/killer_config_v60.json` (v1.0.3唯一权威配置)
...
- **v1.0.3总控中心**:
  ...
  - [scripts/complete_loop_v61.py](scripts/complete_loop_v61.py) — v1.0.3完整闭环,集成总控中心
- **v1.0.3智能优化**:
  ...
  - [scripts/order_execution_engine_v60.py](scripts/order_execution_engine_v60.py) — v1.0.3执行引擎: 去重+TTL撤单+异步任务管理
- **11层闭环核心**:
...
- [references/v1.0.3_INTEGRATION_GUIDE.md](references/v1.0.3_INTEGRATION_GUIDE.md) — **v1.0.3模块集成指南**: 5个高级模块(自适应阈值/修复协议/风控联动/策略熔断/EDF调度)与主流程集成示例代码
...
### 资产
- [assets/configs/killer_config_v60.json](assets/configs/killer_config_v60.json) — v1.0.3唯一权威配置文件
...
- **旧版本模块已归档至 scripts/_archived/ 和 assets/configs/_archived/,仅保留v1.0.3闭环/v1.0.3执行引擎/v1.0.3配置**
